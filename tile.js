...
function tile(source, type, tag, other, fromID)
{
	...
	d3.json(source,function(error,data)
	{
		...
		{
			tileID = "gh_" + data.id + "_" + tag + "_tile";
		}
		...
		else if(type === "gh_comments")
		{
			if(tag == undefined)
			{
				tileID = "gh_" + data.id + "_comments_" + other[1] + "_tile";
			}
			else
			{
				tileID = "gh_" + data.id + "_" + set_strip(tag) + "comments_" + other[1] + "_tile";
			}
		}
					...
					_.keys(data.commits).forEach(function(d,i)
					{
						if(data.commits[d].date == other[1])
						{
							d3.select('#repo_list_' + tileID)
								...
								.html(function()
									{
									    var commentPresence = (data.commits[d].comments > 0) ? ' ' + data.commits[d].comments + ' <img src="media/comment_bubble.png" class="mini-icon">' : '';
										if(data.commits[d].url != undefined)
										{
											...
											return '<p><a href="' + url + '" target="_blank"><b>View Commit</b></a> (' + data.commits[d].repo + ')' + commentPresence + '</p><p>' + data.commits[d].message + '</p><hr>';
										}
										else
										{
